...
/*-----( Declare Variables )-----*/
byte pipe[][6] = {"1Node"}; // Create address for 1 pipe.
...
void setup()   /****** SETUP: RUNS ONCE ******/
{
  ...
  myRadio.openReadingPipe(1, pipe[0]); // Use the first entry in array 'addresses' (Only 1 right now)
...
void loop()   /****** LOOP: RUNS CONSTANTLY ******/
{
  ...
  if ( myRadio.available()) // Check for incoming data from transmitter
  {
    ...
    {
      myRadio.read( &msg, sizeof(msg) ); // Get the data payload (You must have defined that already!)
    }
